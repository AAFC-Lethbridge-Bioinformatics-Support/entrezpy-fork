...
#def get_linkset_unit(elink_cmd):
...
class ElinkAnalyzer(entrezpy.base.analyzer.EutilsAnalyzer):
  ...
  def analyze_result(self, response, request):
    ...
    elif request.cmd in entrezpy.elink.linkset.bare.link_linksets:
      self.analyze_links(response['linksets'], request.cmd)
    elif request.cmd in entrezpy.elink.linkset.bare.list_linksets:
      self.analyze_linklist(response['linksets'], request.cmd)
    else:
      sys.exit(ElinkAnalyzer.logger.error(json.dumps({'error':
        {'unknown elink cmd':request.cmd, 'action':'abort'}})))
    ...
    for i in linksets:
      if 'idurllist' in i:
        for j in i.get('idurllist'):
          lset = entrezpy.elink.linkset.linked.LinkedLinkset(j['id'],
                                                             i['dbfrom'],
                                                             canLink=False)
          ElinkAnalyzer.logger.debug(json.dumps({'created':lset.dump()}))
          for k in j['objurls']:
            lset.add_linkunit(entrezpy.elink.linkset.bare.new_unit(cmd).new(k))
        ...
        for j in i['idchecklist'].get('idlinksets'):
          ...
          for k in j['linkinfos']:
            lset.add_linkunit(
              entrezpy.elink.linkset.bare.new_unit(cmd).new(k['dbto'],
                                                            k['linkname'],
                                                            k.get('menutag'),
                                                            k.get('htmltag'),
                                                            k.get('priority')))
        ...
        for j in i['idchecklist'].get('ids'):
          lset = entrezpy.elink.linkset.linked.LinkedLinkset(j['value'],
                                                             i['dbfrom'],
                                                             canLink=False)
          ElinkAnalyzer.logger.debug(json.dumps({'created':lset.dump()}))
          if 'hasneighbor' in j:
            lset.add_linkunit(entrezpy.elink.linkset.bare.new_unit(cmd).new(i['dbfrom'],
                                                                            j['hasneighbor']))
            ElinkAnalyzer.logger.debug(json.dumps(
              {'added':lset.linkunits[-1].dump()}))
          if 'haslinkout' in j:
  ...
  def analyze_links(self, linksets, cmd):
    ...
    for i in linksets:
      ...
      lset = entrezpy.elink.linkset.linked.LinkedLinkset(i['ids'][0],
                                                         i['dbfrom'])
      ...
      if 'linksetdbs' in i:
        ...
        for j in i['linksetdbhistories']:
          lset.add_linkunit(entrezpy.elink.linkset.bare.new_unit(cmd).new(j['dbto'],
                                                                                  j['linkname'],
                                                                                  i['webenv'],
                                                                                  j['querykey']))
          ElinkAnalyzer.logger.debug(json.dumps(
  ...
  def parse_llinkslib(self, response, cmd, lset=None):
    ...
    for event, elem in xml.etree.ElementTree.iterparse(response, events=['start', 'end']):
      ...
      if event == 'end':
        ...
        if elem.tag == 'ObjUrl':
          isObjurl = False
          lset.add_linkunit(entrezpy.elink.linkset.bare.new_unit(cmd).new(unit))
          ElinkAnalyzer.logger.debug(json.dumps(
            {'added':lset.linkunits[-1].dump()}))
      ...
      if isLinkset and not isObjurl:
        ...
        if elem.tag == 'Id' and not isObjurl:
          lset = entrezpy.elink.linkset.linked.LinkedLinkset(int(elem.text),
                                                             dbfrom,
                                                             canLink=False)
          ElinkAnalyzer.logger.debug(json.dumps({'created':lset.dump()}))
